...
const INDIC_FEATURES: &[(Tag, FeatureFlags)] = &[
    ...
    (Tag::from_bytes(b"nukt"), FeatureFlags::GLOBAL_MANUAL_JOINERS),
    (Tag::from_bytes(b"akhn"), FeatureFlags::GLOBAL_MANUAL_JOINERS),
    (Tag::from_bytes(b"rphf"),        FeatureFlags::MANUAL_JOINERS),
    (Tag::from_bytes(b"rkrf"), FeatureFlags::GLOBAL_MANUAL_JOINERS),
    ...
    (Tag::from_bytes(b"half"),        FeatureFlags::MANUAL_JOINERS),
    (Tag::from_bytes(b"pstf"),        FeatureFlags::MANUAL_JOINERS),
    (Tag::from_bytes(b"vatu"), FeatureFlags::GLOBAL_MANUAL_JOINERS),
    (Tag::from_bytes(b"cjct"), FeatureFlags::GLOBAL_MANUAL_JOINERS),
    // Other features.
    // These features are applied all at once, after final_reordering
    // but before clearing syllables.
    // Default Bengali font in Windows for example has intermixed
    // lookups for init,pres,abvs,blws features.
    (Tag::from_bytes(b"init"),        FeatureFlags::MANUAL_JOINERS),
    ...
    (Tag::from_bytes(b"haln"), FeatureFlags::GLOBAL_MANUAL_JOINERS),
];
...
fn matra_position_indic(u: CodePoint, side: Position) -> Position {
    #[inline] fn in_half_block(u: CodePoint, base: CodePoint) -> bool { u & !0x7F == base }
    #[inline] fn is_deva(u: CodePoint) -> bool { in_half_block(u, 0x0900) }
    #[inline] fn is_beng(u: CodePoint) -> bool { in_half_block(u, 0x0980) }
    #[inline] fn is_guru(u: CodePoint) -> bool { in_half_block(u, 0x0A00) }
    #[inline] fn is_gujr(u: CodePoint) -> bool { in_half_block(u, 0x0A80) }
    #[inline] fn is_orya(u: CodePoint) -> bool { in_half_block(u, 0x0B00) }
    #[inline] fn is_taml(u: CodePoint) -> bool { in_half_block(u, 0x0B80) }
    #[inline] fn is_telu(u: CodePoint) -> bool { in_half_block(u, 0x0C00) }
    #[inline] fn is_knda(u: CodePoint) -> bool { in_half_block(u, 0x0C80) }
    #[inline] fn is_mlym(u: CodePoint) -> bool { in_half_block(u, 0x0D00) }
    #[inline] fn is_sinh(u: CodePoint) -> bool { in_half_block(u, 0x0D80) }

    #[inline]
    fn matra_pos_right(u: CodePoint) -> Position {
        if is_deva(u) {
            Position::AfterSub
        } else if is_beng(u) {
            Position::AfterPost
    ...
    #[inline]
    fn matra_pos_top(u: CodePoint) -> Position {
        if is_deva(u)      {
            Position::AfterSub
        } else if is_guru(u) {
            // Deviate from spec
    ...
    #[inline]
    fn matra_pos_bottom(u: CodePoint) -> Position {
        if is_deva(u) {
            Position::AfterSub
        } else if is_beng(u) {
            Position::AfterSub
...
impl IndicShapePlan {
    fn new(map: &Map, script: Script) -> Self {
        ...
        let is_old_spec = config.has_old_spec && map.chosen_script[0].to_bytes()[3] != b'2';
        ...
        IndicShapePlan {
            ...
            pstf: WouldSubstituteFeature::new(map, Tag::from_bytes(b"pstf"), zero_context),
            mask_array,
        }
    }
...
#[no_mangle]
pub extern "C" fn rb_complex_indic_data_destroy(data: *mut c_void) {
...
#[no_mangle]
pub extern "C" fn rb_indic_get_categories(u: u32) -> u16 {
    let (c1, c2) = get_category_and_position(u);
    c1 as u16 | (c2 as u16) << 8
}

fn final_reordering(plan: &IndicShapePlan, script: Script, buffer: &mut Buffer) {
    if buffer.is_empty() {
        return;
    }
...
fn final_reordering_impl(plan: &IndicShapePlan, script: Script, start: usize, end: usize, buffer: &mut Buffer) {
    ...
    while base < end {
        if buffer.info[base].indic_position() as u32 >= Position::BaseC as u32 {
            ...
            if buffer.props.script == Some(script::MALAYALAM) {
                let mut i = base + 1;
                while i < end {
    ...
    if start + 1 < end && start < base {
        ...
        if buffer.props.script != Some(script::MALAYALAM) && buffer.props.script != Some(script::TAMIL) {
            loop {
        ...
        (buffer.info[start].indic_category() == Category::Repha) ^ buffer.info[start].is_ligated_and_didnt_multiply()
    {
        ...
        loop {
            ...
            // 6. Otherwise, reorder reph to the end of the syllable.
            {
                ...
                if !plan.uniscribe_bug_compatible && buffer.info[new_reph_pos].is_halant() {
    ...
    if buffer.info[start].indic_position() == Position::PreM {
        if start == 0 || (hb_flag_unsafe(buffer.info[start - 1].general_category().to_hb()) & hb_flag_range(ffi::HB_UNICODE_GENERAL_CATEGORY_FORMAT, ffi::HB_UNICODE_GENERAL_CATEGORY_NON_SPACING_MARK)) == 0 {
    ...
    if plan.uniscribe_bug_compatible {
        if !matches!(script, script::TAMIL | script::SINHALA) {
...
pub extern "C" fn rb_complex_indic_initial_reordering(
    ...
    let mut end = buffer.next_syllable(0);
    while start < buffer.len() {
        initial_reordering_syllable(plan, start, end, face, buffer);
        start = end;
        end = buffer.next_syllable(start);
    }
}
...
pub extern "C" fn rb_complex_indic_decompose(
    ...
    if ab == 0x0DDA || (0x0DDC..=0x0DDE).contains(&ab) {
        ...
        let mut ok = false;
        if let Some(g) = font.glyph_index(char::try_from(ab).unwrap()) {
            let g = g.0 as u32;
            ok = plan.pstf.would_substitute(&[g], face);
        }

        if plan.uniscribe_bug_compatible || ok {
...
fn initial_reordering_consonant_syllable(
    ...
    if buffer.props.script == Some(script::KANNADA) &&
        start + 3 <= end &&
    ...
    if plan.is_old_spec {
        let disallow_double_halants = buffer.props.script == Some(script::KANNADA);
        for i in base+1..end {
    ...
    if plan.is_old_spec && buffer.props.script == Some(script::DEVANAGARI) {
...
fn initial_reordering_standalone_cluster(
    ...
    if plan.uniscribe_bug_compatible {
        ...
        if buffer.info[end - 1].indic_category() == Category::DottedCircle {
            return;
        }
    }

    initial_reordering_consonant_syllable(plan, start, end, face, buffer);
}
...
fn consonant_position_from_face(
    ...
    if plan.blwf.would_substitute(&[virama, consonant], face) || plan.blwf.would_substitute(&[consonant, virama], face) {
...
fn initial_reordering_syllable(
    ...
    match syllable_type {
        ...
        SyllableType::BrokenCluster | SyllableType::StandaloneCluster => {
            initial_reordering_standalone_cluster(plan, start, end, face, buffer);
        }
...
fn update_consonant_positions(
    ...
    if plan.config.base_pos != BasePosition::Last {
        return;
    }
